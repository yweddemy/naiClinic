...
/*!***********************!*\
  ...
  \***********************/
/*! no static exports found */
(function(module,exports){})});var aiEnableCookie=!1;var aiId='';var aiExtraSpace=0;var aiAccTime=0;var aiOnloadEventsCounter=0;var aiCallbackExists=typeof aiReadyCallbacks!=='undefined'&&aiReadyCallbacks instanceof Array;var aiReadyCallbacks=aiCallbackExists?aiReadyCallbacks:[];function aiDebugExtended(message){if(typeof aiShowDebug!=='undefined'&&aiShowDebug){if(console&&console.log){console.log("Advanced iframe: "+message)}}}
...
if(obj.contentWindow.document.body!=null){var oldScrollposition=jQuery(window).scrollTop();obj.style.marginTop=0;obj.style.marginBottom=0;obj.height=Number(resizeMinHeight);obj.style.height=Number(resizeMinHeight)+'px';var newheight=aiGetIframeHeight(obj);aiDebugExtended("aiResizeIframe - newheight: "+newheight);obj.height=newheight;obj.style.height=newheight+'px';if(jQuery('#ai-zoom-div-'+obj.id).length!==0){var zoom=window['zoom_'+obj.id];jQuery('#ai-zoom-div-'+obj.id).css('height',newheight*zoom)}
...
jQuery('#ai h2,#ai .icon_ai,#ai h3,#ai h4').not('.show-always').hide();jQuery('#ai .form-table').addClass('ai-remove-margin');jQuery('#ai hr, .signup_account_container, .config-file-block').hide();jQuery('#ai .hide-always').hide();jQuery('#ai .hide-search').hide();jQuery('#ai .postbox-container').not('.show-always').hide()}else{jQuery('#ai p').not('.form-table p').show();jQuery('#ai section .ai-anchor').show();jQuery('#ai ul').not('.form-table ul').show();jQuery('#ai ol').not('.form-table ol').show();if(accType!=='false'){jQuery('#ai h1').not('.show-always').show();jQuery('#ai #acc').attr('id','accordion');jQuery('#ai #accordion > div').hide();jQuery('#ai #spacer-div').hide()}
jQuery('#ai h2,#ai .icon_ai,#ai h3,#ai h4').not('.show-always').show();jQuery('#ai .form-table').removeClass('ai-remove-margin');jQuery('#ai hr, .signup_account_container, .config-file-block').show();jQuery('#ai .sub-domain-container').show();jQuery('#ai .hide-search').show();jQuery('#ai .hide-always').hide();jQuery('#ai .postbox-container').show();setTimeout(function(){jQuery('#ai .postbox-container .closed .inside').css('display','')},5)}
jQuery('#ai .mark-tab-header').removeClass('mark-tab-header');var firstHit='';jQuery('#ai tr').each(function(){var $this=jQuery(this);var valueLabel=$this.find('th').text();var valueDescription=$this.find('p.description').text();valueLabel=(valueLabel!==undefined)?valueLabel.toLowerCase():'XXXXXXX';valueDescription=(valueDescription!==undefined)?valueDescription.toLowerCase():'XXXXXXX';if(valueLabel.indexOf(searchTerm)===-1&&valueDescription.indexOf(searchTerm)===-1){$this.addClass('hide-setting')}else{$this.closest('table').prevAll('h2:first').show();$this.closest('.postbox-container').show();$this.closest('.postbox-container').find('h2, .inside').show();$this.closest('table').prevAll('#ai .icon_ai:first').show();$this.closest('table').nextAll('p.button-submit:first').show();$this.removeClass('hide-setting');$this.closest('.hide-search').show();if(searchTerm.length>2){var headerId=$this.closest('section').attr('class');if(headerId!==undefined){jQuery('#'+headerId).addClass('mark-tab-header');if(firstHit===''){firstHit=headerId}}}
...
function aiGenerateShortcode(isPro){var output='[advanced_iframe ';if(jQuery('#securitykey').val()!==''){output+='securitykey="'+jQuery('#securitykey').val()+'" '}
...
return parentWidth}
function aiSetZoom(id,zoom){var obj=jQuery('#'+id);if(aiIsIe8===!0){obj.css('-ms-zoom',zoom)}
...
var match=f.match(/^(\-){0,1}([\d.])+(px|%|em|pt|vh|vw|rem|ch)?(\-|\+){0,1}([\d.]){0,7}(px|%|em|pt|vh|vw|rem|ch)?$/);if(!match){alert('Please check the value you have entered. Only numbers with a dot or with an optional px, %, em or pt are allowed.');setTimeout(function(){inputField.focus()},10)}}
...
var lengthTrue=f.length===36;var minusFourTrue=(f.match(/-/g)||[]).length===4;if(!lengthTrue||!minusFourTrue){alert('Please check the value you have entered. Your input seems not to be a valid purchase code.');setTimeout(function(){inputField.focus()},10)}}
...
function aiShowLayerIframe(event,id,path,hideUntilLoaded,showLoadingIcon,keep,reload){aiDebugExtended("aiShowLayerIframe");keep=(keep===undefined)?!1:keep;reload=(reload===undefined)?!0:reload;var layerId='#'+id;jQuery('#ai-zoom-div-'+id).show();if(reload&&hideUntilLoaded==='true'){jQuery(layerId).css('visibility','hidden')}
...
function aiHideLayerIframe(id,keep){aiDebugExtended("aiHideLayerIframe");jQuery('#'+id).hide();if(!keep){jQuery('#'+id).attr('src','about:blank');aiLayerIframeHrefs[id]='about:blank'}
...
if(loc!==encodeURIComponent(orig)){newUrl=aiSetGetParameter(param,loc);var removeProtocol=!0;if(prefix==='hash'||prefix==='hashrewrite'){var locBase=aiRemoveQueryString(loc);return aiGetUrlMapping(locBase,param,prefix)}else if(prefix){var repUrl=newUrl.replace(prefix,'');if(repUrl===newUrl){removeProtocol=!1}
newUrl=repUrl}
...
if(isDirect){var newUrl=aiRemoveQueryString(window.location.href);var locDecoded=decodeURIComponent(loc);var queryStart=locDecoded.indexOf('?');if(queryStart!==-1){var queryPart=locDecoded.slice(queryStart+1);newUrl+='?'+queryPart;keepSlash=!0}}
...
function aiGetUrlMapping(url,param,prefix){var data={action:'aip_map_url_action',security:MyAjax.security,url:url};jQuery.post(MyAjax.ajaxurl,data,function(id){var newUrl=aiGetUrlMappingUrl(url,param,prefix,id);aiSetBrowserUrl(newUrl,!1)})}
function closeInfoPermanent(id){var data={action:'aip_close_message_permanent',security:MyAjax.security,id:id};var message='The message before will only appear again when you reset the advanced iframe settings.';if(id==='show-discount-message'){message='The message of advanced iframe shown before will only appear again when you reset the advanced iframe settings or a new discount is available.'}else if(id==='show-registration-message'){message='The message before will only appear again when you reset the advanced iframe settings and you have not entered your purchase code.'}
jQuery.post(MyAjax.ajaxurl,data,function(id){jQuery('h1').after('<div class="message-notice notice notice-success"><p>'+message+'</p></div>')});setTimeout(function(){jQuery(".message-notice").fadeOut()},4000)}
...
jQuery(document).ready(function(){aiDebugExtended("document.ready called");jQuery('iframe').parent('p').css('margin','0');aiWindowWidth=jQuery(window).width();jQuery.each(aiReadyCallbacks,function(index,callback){callback()});setTimeout(function(){jQuery("#ai #ai-updated-text").css("visibility","hidden")},4000);jQuery('#ai #checkIframes').on('click',function(){jQuery('.ai-spinner').css('display','inline-table');jQuery(this).addClass('disabled');setTimeout(aiDisableCheckIframes,200)});var moved=!1;jQuery('#aiDebugDivTotal').mousedown(function(){moved=!1}).mousemove(function(){moved=!0}).mouseup(function(event){if(!moved){var elem=jQuery('#aiDebugDiv');if(Math.floor(elem.height())>'300'){elem.height('0px')}else{elem.height('400px')}}});if(typeof ai_show_id_only!=='undefined'){if(jQuery('#'+ai_show_id_only).length===0){alert('The element with the id "'+ai_show_id_only+'" cannot be found. Please check your configuration.')}else{var element=jQuery('#'+ai_show_id_only);element.siblings().hide();var parents=element.parents();parents.siblings().hide();parents.css('padding','0px').css('margin','0px').css('overflow','hidden');if(parent===top){var elementRaw=element[0];elementRaw.style.marginTop=elementRaw.style.marginBottom=0;elementRaw.style.overflow="hidden";var newHeightRaw=Math.max(elementRaw.scrollHeight,elementRaw.offsetHeight);var newHeight=parseInt(newHeightRaw,10);var data={'aitype':'height','height':newHeight,'id':ai_show_id_only};var json_data=JSON.stringify(data);window.parent.postMessage(json_data,'*')}}}});function aiDisableCheckIframes(){var input=jQuery("<input>").attr("type","hidden").attr("name","checkIframes").val("true");jQuery("#ai_form").append(input);jQuery("#ai_form").submit();jQuery('#checkIframes').prop('disabled','disabled')}
function aiProcessMessage(event,id,debug){var jsObject;try{var jsObject=JSON.parse(event.data)}catch(e){if(debug==='debug'&&console&&console.log){console.log('Advanced iframe: The received message cannot be parsed and seems not to belong to advanced iframe pro. Please disable the postMessage debug mode if this o.k. and that this message is not shown anymore.');console.log("Advanced iframe: Unknown event: ",event)}
...
function aiProcessDebug(jsObject){var debugData=jsObject.data;if(jQuery('#aiDebugDiv').length!==0){debugData=debugData.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");debugData=debugData.replace('\"','"').replace(/\\/g,"");jQuery('#aiDebugDiv').append('<p class="ai-debug-remote"> r: '+debugData+'</p>')}}
...
function aiRemoveElementsFromHeight(id,height,removeElements){var iframe=jQuery('#'+id);var elementArray=removeElements.split(',');var totalHeight=0;for(var i=0;i<elementArray.length;i++){try{var el=elementArray[i];if(el.includes("|")){var rangeArray=el.split('|');var bottomElement=jQuery(rangeArray[0]);var beforeBottom=Math.round(bottomElement.offset().top+bottomElement.outerHeight(!0));var nextTop=Math.round(jQuery(rangeArray[1]).offset().top);totalHeight+=nextTop-beforeBottom}else if(el==='top'){totalHeight+=Math.round(jQuery('#'+id).offset().top)}else if(isNaN(el)){totalHeight+=jQuery(el).outerHeight(!0)}else{totalHeight+=parseInt(el)}}catch(e){if(console&&console.error){console.error('Advanced iframe configuration error: The configuration of remove_elements_from_height "'+removeElements+'" is invalid. Please check if the elements you defined do exist and ids/classes are defined properly.');console.log(e)}}}
var calc='calc('+height+' - '+totalHeight+'px)';iframe.css('height',calc)}
